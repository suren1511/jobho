...
<?else:?>
    ...
    $points = '<li><span>...</span></li>';

	$arResult["nStartPage"]++;
	while($arResult["nStartPage"] <= $arResult["nEndPage"]-1):
	?>
        <?if ($startPoints && $arResult["nStartPage"]>3 && $arResult["NavPageNomer"]>3 && $arResult["NavPageNomer"]!=4) {
            echo $points;
            $startPoints = false;
        } ?>
		<?if ($arResult["nStartPage"] == $arResult["NavPageNomer"]):?>
		...
		<?else:
        if ($startPoints && $arResult["NavPageNomer"]==4) {
            echo $points;
            $startPoints = false;
        }
        ?>
	...
	<?if($arResult["NavPageNomer"] < $arResult["NavPageCount"]):?>
		<?if($arResult["NavPageCount"] > 1):?>
            <?if ($arResult["NavPageNomer"] < ($arResult["NavPageCount"]-2)) echo $points;?>

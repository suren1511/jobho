    ...
    <div class="form-groups">
      ...
      <? if (is_array($arResult["PROPERTY_LIST"]) && count($arResult["PROPERTY_LIST"] > 0)): ?>
        ...
        <? foreach ($arResult["PROPERTY_LIST"] as $propertyID): ?>
          ...
          <? if ($propertyID != '526' && $propertyID != '529') { ?>
            <div class="form-group"<? if ($propertyID == 'NAME' || $propertyID == '531') { ?> style="display: none" <? } ?>>
          ...
          switch ($INPUT_TYPE):
            ...
            case "N":
              for ($i = 0; $i < $inputNum; $i++) {
                ...
                if ($propertyID == 'NAME'){
                  $value = $USER->GetFullName().' - '.date('d.m.Y');
                }
                ?>
                  ...
                  foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum) {
                    ...
                    <div class="form-group">
                      <div class="form-group-mini col-4">
                        ...
                        ?>/>
                        <label for="property_<?= $key ?>"><b><?= $arEnum["VALUE"] ?></b>
                          <? if ($key == '2473') { ?>
                            <span>Вопросы и консультации по&nbsp;работе с&nbsp;сайтом, техническая поддержка и&nbsp;предложения по&nbsp;улучшению сайта и&nbsp;сервисов</span>
                            <?
                          } ?>
                          <? if ($key == '2474') { ?>
                            <span>Вопросы по&nbsp;подбору услуг и&nbsp;сервисов.</span>
                            <?
                          } ?>
                          <? if ($key == '2475') { ?>
                            <span>Вопросы по&nbsp;подбору услуг и&nbsp;сервисов.</span>
        ...
        <? } ?>
          <? if ($propertyID != '526' && $propertyID != '528') { ?>
            </div>
            </div>
          <? } ?>
        <? endforeach; ?>

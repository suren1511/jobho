    ...
    <div class="form-groups">
      ...
      <? if (is_array($arResult["PROPERTY_LIST"]) && count($arResult["PROPERTY_LIST"] > 0)): ?>
        ...
        <? foreach ($arResult["PROPERTY_LIST"] as $propertyID): ?>
          ...
          <div class="form-group<? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]['MULTIPLE']>='Y'){?> align-top<?}?>"<? if ($arResult["PROPERTY_LIST_FULL"][$propertyID]['SORT']>='1000'){?> style="display: none;" <?}?>>
            <label>
                    ...
                    switch ($type):
                      ...
                      case "radio":
                        ...
                        foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum) {
                          ...
                          ?>
                          <div class="form-group-mini<? if ($countList<4){?> col-2 col-xs-3<?}?>">
                            <input type="<?= $type ?>" name="PROPERTY[<?= $propertyID ?>]<?= $type == "checkbox" ? "[" . $key . "]" : "" ?>" value="<?= $key ?>" id="property_<?= $key ?>"<?= $checked ? " checked=\"checked\"" : "" ?> class="styled"<?

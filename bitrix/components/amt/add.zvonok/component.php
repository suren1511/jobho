		...
		if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
		{
			...
			foreach ($arParams["PROPERTY_CODES"]  as $i => $propertyID)
			{
				...
				if (intval($propertyID) > 0)
				{
					...
					if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"])
					{
					...
					else
					{
						...
						foreach ($arPropertyValue as $key => $value)
						{
							$arFile = $_REQUEST["PROPERTY_FILE_".$propertyID."_".$key];
              //$arFile =
          ...
          $arUpdateValues['CODE'] = CUtil::translit($name, "ru", $params);
          $arSelect = Array("ID", "IBLOCK_ID", "NAME");
          $arFilter = Array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "CODE" => $arUpdateValues['CODE']);
          ...
          if ($nalId) {
            ...
            $arUpdateValues['CODE'] = CUtil::translit($name . $new_password, "ru", $params);
          }
          else{
            $arUpdateValues['CODE'] = CUtil::translit($name, "ru", $params);
          }
